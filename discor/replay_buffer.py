...
class ReplayBuffer:
    ...
    def _reset(self):
        ...
        self._select_nums = np.empty((self._memory_size, 1), dtype=np.float32)
        if self._arbi_reset:
    ...
    def _append(self, state, action, reward, next_state, done, step=None, select_nums=None, sim_state=None, episode_done=None):
        ...
        self._select_nums[self._p, ...] = select_nums
        if self._arbi_reset:
    ...
    def _sample_batch(self, idxes, batch_size, device):
        ...
        self._select_nums[idxes] += 1
        select_nums = torch.tensor(
            self._select_nums[idxes], dtype=torch.float, device=device)
        batch = {
            'states': states,
            'actions': actions,
            'rewards': rewards,
...
class TemporalPrioritizedReplayBuffer(ReplayBuffer):
    ...
    def _reset(self):
        ...
        self._select_nums = np.empty((self._memory_size, 1), dtype=np.int64)
        self._done_cnts = np.empty((self._memory_size, 1), dtype=np.int64)
        self._cur_done_cnt = 0

    def _append(self, state, action, reward, next_state, done, step, select_nums, sim_state=None, episode_done=None):
        super()._append(state, action, reward, next_state, done, step, select_nums, sim_state, episode_done=episode_done)
...
class BackTimeBuffer(TemporalPrioritizedReplayBuffer):
    ...
    def append(self, state, action, reward, next_state, done, step=None, episode_done=None, select_nums=None, sim_state=None):
        ...
        self._append(state, action, reward, next_state, done, 0, 0, sim_state)
