...
class SAC(Algorithm):
    ...
    def explore(self, state):
        ...
        with torch.no_grad():
            action, _, _, _ = self._policy_net(state)
    ...
    def exploit(self, state):
        state = torch.tensor(
            state[None, ...].copy(), dtype=torch.float, device=self._device)
        with torch.no_grad():
            _, _, action, _ = self._policy_net(state)
        action = action.cpu().numpy()[0]
        assert_action(action)
        return action
    ...
    def update_policy_and_entropy(self, batch, writer):
        ...
        # Update policy.
        policy_loss, entropies, weight_entropy = self.calc_policy_loss(states)
    ...
    def calc_policy_loss(self, states):
        ...
        sampled_actions, entropies, _, weight_entropy = self._policy_net(states)
        ...
        return policy_loss, entropies.detach_(), weight_entropy.detach_()
    ...
    def update_q_functions(self, batch, writer, fast_batch=None, err_preds=None, weight_entropy=None, episodes=None):
        ...
        q_loss, mean_q1, mean_q2, unweighted_q_loss = \
            self.calc_q_loss(curr_qs1, curr_qs2, target_qs, weight_entropy, episodes)
    ...
    def calc_target_qs(self, rewards, next_states, dones):
        with torch.no_grad():
            next_actions, next_entropies, _, _ = self._policy_net(next_states)
    ...
    def calc_q_loss(self, curr_qs1, curr_qs2, target_qs, weight_entropy=None, episodes=None):
        ...
        # else:
        q1_loss = torch.mean((curr_qs1 - target_qs).pow(2) * weight_entropy)
        q2_loss = torch.mean((curr_qs2 - target_qs).pow(2) * weight_entropy)
