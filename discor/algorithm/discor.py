...
class DisCor(SAC):
    ...
    def update_online_networks(self, batch, writer, episodes):
        self._learning_steps += 1
        weight_entropy = self.update_policy_and_entropy(batch['uniform'], writer)
        self.update_q_functions_and_error_models(batch, writer, weight_entropy, episodes)
    ...
    def update_q_functions_and_error_models(self, batch, writer, weight_entropy, episodes):
        ...
        # weight_entropy = torch.ones((128, 1)).to(device=self._device)
        ...
        if self.lfiw:
            ...
            weight_entropy *= lfiw_weights

        AMO_steps = train_batch["steps"].to(dtype=torch.float32)
        AMO_select_nums = train_batch["select_nums"]
        coef_weight = torch.exp(0.996 ** (AMO_steps + AMO_select_nums)) / (math.e - 1)
        coef_weight = coef_weight / torch.sum(coef_weight) * AMO_steps.shape[0]
        weight_entropy *= coef_weight
        ...
        curr_qs1, curr_qs2, target_qs = \
            self.update_q_functions(train_batch, writer, fast_batch, curr_errs1, weight_entropy, episodes)
