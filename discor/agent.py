...
class Agent:
    ...
    def __init__(self, env, test_env, algo: Algorithm, log_dir, device, num_steps=3000000,
        ...
        if hasattr(algo, "lfiw") and algo.lfiw:
            if not fast_memory_size:
                fast_memory_size = memory_size // 5
            self.lfiw = True
    ...
    def run(self):
        episode_return_max = -10000
        while True:
            temp_return = self.train_episode(episode_return_max)
            episode_return_max = temp_return
            if self._steps > self._num_steps:
                np.savetxt('Data.csv', DataCsv, delimiter=',')
                break

    def train_episode(self, episode_return_max=None):
        self._episodes += 1
        episode_return = 0.
        episode_steps = 1
        select_nums = 0
        ...
        while (not done):
            ...
            transition = [state, action, reward, next_state, masked_done, episode_steps, done, select_nums]
            if self._eval_tper:
            ...
            if self._steps >= self._start_steps:
                ...
                if self._steps % self._update_interval == 0:
                    ...
                    self._algo.update_online_networks(batch, self._writer, self._episodes)
        ...
        if episode_return >= episode_return_max:
